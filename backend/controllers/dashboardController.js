...
exports.getDashboardData = async (req, res) => {
  try {
    ...
    const last60DaysExpenseTranscations = await Expense.find({
      userId,
      date: { $gte: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000) },
    }).sort({ date: -1 });

    //Get total expenses for last 60 days
    const expenseLast60Days = last60DaysExpenseTranscations.reduce(
      (total, expense) => total + expense.amount,
      0
    );

    //Fetch last 5 days transactions (income + expenses)
    ...
    res.status(200).json({
      ...
      last60DaysExpenses: {
        ...
        transactions: last60DaysExpenseTranscations,
      },
      last60DaysIncomes: {
